...
RUN <<EOT
#    apt-get install --assume-yes --no-install-recommends llvm-dev libclang-dev clang
...
ENV ZEPHYR_EXTRA_MODULES=/workdir/zephyr-rust
COPY config.toml /root/.cargo/

RUN <<EOT
    . /root/.cargo/env &&
    cd /workdir/zephyr-rust/rust &&
    for x in zephyr*; do
        cd $x && cargo fetch && cd ..
    done
    cd /workdir/zephyr-rust/zephyr-bindgen
    cargo fetch
EOT
